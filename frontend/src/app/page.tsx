          ...
          <div style={{ display: activeTab === "report" ? undefined : "none" }}>
            ...
            <div className="flex items-center justify-between mb-4 no-print">
              <p className="text-sm text-slate-500">
                Report preview — export as PDF or Word
              </p>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    const today = new Date().toISOString().slice(0, 10);
                    printTitleRef.current = `${result?.address ?? "Property Report"} ${today}`;
                    window.print();
                  }}
                  className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-5 py-2.5 text-sm font-medium text-cyan-400 shadow-sm hover:bg-slate-800 transition-colors border border-cyan-500/30"
                >
                ...
                <button
                  onClick={() => {
                    ...
                    exportWordReport({
                      ...
                      built_form: result.built_form,
                      floor_area_m2: result.floor_area_m2,
                      num_rooms: result.num_rooms,
                      construction_age_band: result.construction_age_band,
                      heating_type: result.heating_type,
                      energy_rating: result.energy_rating,
                      energy_score: result.energy_score,
                      council_tax_band: result.council_tax_band,
                      lsoa: result.lsoa,
                      lat: result.lat,
                      lon: result.lon,
                      tenure: result.tenure,
                      lease_commencement: result.lease_commencement,
                      lease_expiry_date: result.lease_expiry_date,
                      lease_term_years: result.lease_term_years,
                      rivers_sea_risk: result.rivers_sea_risk,
                      surface_water_risk: result.surface_water_risk,
                      planning_flood_zone: result.planning_flood_zone,
                      listed_buildings: result.listed_buildings ?? [],
                      conservation_areas: result.conservation_areas ?? [],
                      green_belt: result.green_belt,
                      aonb: result.aonb,
                      brownfield: result.brownfield ?? [],
                      coal_mining_high_risk: result.coal_mining_high_risk,
                      coal_mining_in_coalfield: result.coal_mining_in_coalfield,
                      radon_risk: result.radon_risk,
                      sssi: result.sssi ?? [],
                      ancient_woodland: result.ancient_woodland ?? [],
                      ground_shrink_swell: result.ground_shrink_swell,
                      ground_landslides: result.ground_landslides,
                      ground_compressible: result.ground_compressible,
                      ground_collapsible: result.ground_collapsible,
                      ground_running_sand: result.ground_running_sand,
                      ground_soluble_rocks: result.ground_soluble_rocks,
                      sales: result.sales ?? [],
                      comparables: adoptedComparables.map((c, i) => ({
                        address: c.address,
                        property_type: c.property_type,
                        building_era: c.building_era,
                        bedrooms: c.bedrooms,
                        price: c.price,
                        floor_area_sqm: c.floor_area_sqm,
                        transaction_date: c.transaction_date,
                        adjFactor: adjFactors[i] ?? 1,
                        sizeAdjPsf: sizeAdjResults[i]?.adjPsf ?? null,
                      })),
                      adoptedPriceMin, adoptedPriceMax, adoptedPriceAvg,
                      adoptedPsfMin, adoptedPsfMax, adoptedPsfAvg,
                      adjPsfMin, adjPsfMax, adjPsfAvg,
                      sizeAdjPsfMin, sizeAdjPsfMax, sizeAdjPsfAvg,
                      adoptedSizeMin, adoptedSizeMax, adoptedSizeAvg,
                      adoptedDateMin, adoptedDateMax,
                      indicativeLow: low ?? null,
                      indicativeHigh: high ?? null,
                      indicativeMid: mid ?? null,
                      subjectAreaM2,
                      hpiCorrelation,
                      sizeElasticity,
                    });
                  }}
                  className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-5 py-2.5 text-sm font-medium text-blue-400 shadow-sm hover:bg-slate-800 transition-colors border border-blue-500/30"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* ── Report document ─────────────────────────────────────────── */}
            <div className="propval-report bg-white shadow-xl max-w-4xl mx-auto mb-12" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", "Helvetica Neue", Arial, sans-serif', borderRadius: "12px", overflow: "hidden", WebkitPrintColorAdjust: "exact", printColorAdjust: "exact" }}>

              {/* Print table wrapper — on screen the thead is hidden (display:none in CSS).
                  In print, <thead> natively repeats on every page in all browsers. */}
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead className="propval-print-thead">
                  <tr>
                    <th style={{ padding: 0, fontWeight: "normal", textAlign: "left" }}>
                      <div style={{ background: "linear-gradient(135deg, #007AFF 0%, #5856D6 100%)", padding: "16px 18mm", display: "flex", alignItems: "center", justifyContent: "space-between", WebkitPrintColorAdjust: "exact", printColorAdjust: "exact" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <span style={{ fontSize: "11px", fontWeight: 700, color: "rgba(255,255,255,0.75)", letterSpacing: "0.12em", textTransform: "uppercase" }}>PropVal</span>
                          <span style={{ display: "inline-block", width: 1, height: 16, background: "rgba(255,255,255,0.35)" }} />
                          <span style={{ fontSize: "15px", fontWeight: 600, color: "white" }}>Valuation Report</span>
                        </div>
                        <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.7)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "55%" }}>
                          {result.address}
                        </span>
                      </div>
                      <div style={{ height: 18 }} />
              ...
              {/* Cover bar — screen only; replaced by thead header in print */}
              <div className="propval-report-cover" style={{ background: "linear-gradient(135deg, #007AFF 0%, #5856D6 100%)", padding: "32px 48px" }}>
                <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", margin: "0 0 8px", textTransform: "uppercase" }}>PropVal</p>
                <h1 style={{ color: "#FFFFFF", fontSize: "30px", fontWeight: 700, margin: 0, letterSpacing: "-0.02em", lineHeight: 1.1 }}>Valuation Report</h1>
              </div>

              <div className="propval-report-body" style={{ padding: "40px 48px" }}>

                {/* Subject property header */}
                <div style={{ paddingBottom: "24px", marginBottom: "32px", borderBottom: "1px solid #E5E5EA" }}>
                  <p style={{ fontSize: "11px", color: "#8E8E93", fontWeight: 500, margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Subject Property</p>
                  <h2 style={{ fontSize: "20px", fontWeight: 700, color: "#000000", margin: "0 0 4px", lineHeight: 1.3, letterSpacing: "-0.01em" }}>{result.address}</h2>
                  {result.uprn && <p style={{ fontSize: "11px", color: "#8E8E93", margin: "0 0 16px" }}>UPRN {result.uprn}</p>}
                  <div style={{ display: "flex", gap: "28px", flexWrap: "wrap" }}>
                    {[
                      ["Valuation Date", valuationDate ? new Date(valuationDate).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : "Not specified"],
                      ["Report Date", new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })],
                      result.admin_district ? ["Local Authority", result.admin_district] : null,
                      result.region       ? ["Region", result.region]           : null,
                    ].filter((x): x is [string, string] => Boolean(x)).map(([l, v]) => (
                      <div key={String(l)} style={{ borderLeft: "2px solid #007AFF", paddingLeft: "8px" }}>
                        <p style={{ fontSize: "10px", color: "#8E8E93", fontWeight: 500, margin: "0 0 2px", textTransform: "uppercase", letterSpacing: "0.04em" }}>{String(l)}</p>
                        <p style={{ fontSize: "13px", fontWeight: 600, color: "#000000", margin: 0 }}>{String(v)}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* ── 1. Property Details ── */}
                <div className="propval-report-section" style={rptSection}>
                  <h2 style={rptH2}>1. PROPERTY DETAILS</h2>
                  <table style={rptTable}>
                    <tbody>
                      {([
                        ...
                        ["Habitable Rooms",  result.num_rooms != null ? String(result.num_rooms) : null],
                        ["Construction Era", result.construction_age_band],
                        ["Heating",          result.heating_type],
                        ["EPC Rating",       result.energy_rating && result.energy_score != null ? `${result.energy_rating}  (score ${result.energy_score})` : result.energy_rating],
                        ["Council Tax Band", result.council_tax_band ? `Band ${result.council_tax_band}` : null],
                        ["LSOA",             result.lsoa],
                        ["Coordinates",      result.lat != null ? `${result.lat.toFixed(5)}, ${result.lon!.toFixed(5)}` : null],
                      ] as [string, string | null][]).filter(([, v]) => v != null).map(([label, value], i) => (
                        <tr key={label} style={rptStripe(i)}>
                          <td style={rptTdL}>{label}</td>
                          <td style={rptTdV}>{value}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {result.tenure && (
                  <div className="propval-report-section" style={rptSection}>
                    <h2 style={rptH2}>2. TENURE</h2>
                    <table style={rptTable}>
                      <tbody>
                        {([
                          ["Tenure", result.tenure],
                          result.lease_commencement ? ["Lease Commencement", fmtDate(result.lease_commencement)] : null,
                          result.lease_expiry_date  ? ["Lease Expiry",       fmtDate(result.lease_expiry_date)]  : null,
                          result.lease_expiry_date  ? ["Unexpired Term",     yearsMonths(new Date(), new Date(result.lease_expiry_date))] : null,
                          result.lease_term_years   ? ["Original Term",      `${result.lease_term_years} years`] : null,
                        ] as ([string, string] | null)[]).filter((x): x is [string, string] => Boolean(x)).map(([label, value], i) => (
                          <tr key={label} style={rptStripe(i)}>
                            <td style={rptTdL}>{label}</td>
                            <td style={rptTdV}>{value}</td>
                          </tr>
                ...
                <div className="propval-report-section" style={rptSection}>
                  <h2 style={rptH2}>3. ENVIRONMENTAL &amp; STATUTORY ASSESSMENT</h2>
                  <table style={rptTable}>
                    <thead>
                      <tr>
                        <th style={{ ...rptTh, width: "35%" }}>Factor</th>
                        <th style={rptTh}>Finding</th>
                        <th style={{ ...rptTh, width: "22%" }}>Source</th>
                      </tr>
                    </thead>
                    <tbody>
                      {([
                        ["Flood Risk — Rivers &amp; Sea",    result.rivers_sea_risk ?? "Not assessed",                                "EA NaFRA2 Jan 2025"],
                        ["Flood Risk — Surface Water",       result.surface_water_risk ?? "Not assessed",                             "EA Surface Water"],
                        ["NPPF Planning Flood Zone",         result.planning_flood_zone ?? "Zone 1 (Low probability)",                "planning.data.gov.uk"],
                        ["Listed Buildings (75 m)",          (result.listed_buildings ?? []).length === 0
                                                               ? "None within 75 m"
                                                               : (result.listed_buildings ?? []).map(lb => `${lb.grade}: ${lb.name}`).join(" | "),
                                                             "Historic England NHLE"],
                        ["Conservation Area",                (result.conservation_areas ?? []).length === 0
                                                               ? "Not within a conservation area"
                                                               : (result.conservation_areas ?? []).map(ca => ca.name).join(", "),
                                                             "planning.data.gov.uk"],
                        ["Green Belt",                       result.green_belt ? "Within Green Belt" : "Not in Green Belt",          "Natural England"],
                        ["AONB / National Landscape",        result.aonb ?? "Not within an AONB",                                    "Natural England"],
                        ["Brownfield Land (100 m)",          (result.brownfield ?? []).length === 0
                                                               ? "No brownfield sites within 100 m"
                                                               : `${(result.brownfield ?? []).length} site(s) identified within 100 m`,
                                                             "planning.data.gov.uk"],
                        ["Coal Mining — High Risk Area",     result.coal_mining_high_risk ? "Within Development High Risk Area" : "Not in High Risk Area", "Mining Remediation Authority"],
                        ["Coal Mining — Coalfield",          result.coal_mining_in_coalfield ? "Within coalfield area" : "Not in coalfield area",           "Mining Remediation Authority"],
                        ["Radon Risk",                       result.radon_risk ?? "Not assessed",                                    "UK Radon / PHE"],
                        ["SSSI (2 km)",                      (result.sssi ?? []).length === 0
                                                               ? "No SSSIs within 2 km"
                                                               : (result.sssi ?? []).join(" | "),
                                                             "Natural England"],
                        ["Ancient Woodland (50 m)",          (result.ancient_woodland ?? []).length === 0
                                                               ? "No ancient woodland within 50 m"
                                                               : (result.ancient_woodland ?? []).map(aw => aw.name).join(" | "),
                                                             "Natural England"],
                  ...
                  {(result.ground_shrink_swell || result.ground_landslides || result.ground_compressible || result.ground_collapsible || result.ground_running_sand || result.ground_soluble_rocks) && (
                    <div style={{ marginTop: "12px" }}>
                      <p style={{ fontSize: "11px", fontWeight: 600, color: "#6E6E73", letterSpacing: 0, marginBottom: "8px" }}>Ground Conditions — BGS GeoSure</p>
                      <table style={rptTable}>
                        <thead>
                          <tr>
                            <th style={{ ...rptTh, width: "45%" }}>Hazard</th>
                            <th style={rptTh}>Susceptibility</th>
                          </tr>
                        </thead>
                        <tbody>
                          {([
                            ["Shrink-swell clay",   result.ground_shrink_swell],
                            ["Landslides",          result.ground_landslides],
                            ["Compressible ground", result.ground_compressible],
                            ["Collapsible deposits",result.ground_collapsible],
                            ["Running sand",        result.ground_running_sand],
                            ["Soluble rocks",       result.ground_soluble_rocks],
                          ] as [string, string | null][]).filter(([, v]) => v != null).map(([label, value], i) => (
                            <tr key={label} style={rptStripe(i)}>
                              <td style={rptTdL}>{label}</td>
                              <td style={rptTdV}>{value}</td>
                ...
                <div className="propval-report-section" style={rptSection}>
                  <h2 style={rptH2}>4. TRANSACTION HISTORY</h2>
                  <p style={{ fontSize: "10px", color: "#94A3B8", marginBottom: "8px", fontFamily: "'Courier New', Courier, monospace" }}>Source: HM Land Registry Price Paid Data</p>
                  {(result.sales ?? []).length === 0 ? (
                    <p style={{ fontSize: "12px", color: "#999", fontStyle: "italic" }}>No Land Registry transactions found for this address.</p>
                  ) : (
                    <table style={rptTable}>
                      <thead>
                        <tr>
                          {["Date", "Price", "Tenure", "Property Type"].map(h => <th key={h} style={rptTh}>{h}</th>)}
                        </tr>
                      </thead>
                      <tbody>
                        {(result.sales ?? []).map((sale, i) => (
                          <tr key={i} style={rptStripe(i)}>
                            <td style={rptTdV}>{sale.date}</td>
                            <td style={{ ...rptTdV, fontWeight: 700 }}>{formatPrice(sale.price)}</td>
                            <td style={rptTdV}>{sale.tenure}</td>
                            <td style={rptTdV}>{sale.property_type}{sale.new_build ? " (New Build)" : ""}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="propval-report-section" style={rptSection}>
                  <h2 style={rptH2}>5. SALES COMPARABLE EVIDENCE</h2>
                  <p style={{ fontSize: "11px", color: "#86868B", marginBottom: "8px" }}>
                    Source: HM Land Registry Price Paid Data — {adoptedComparables.length} comparable{adoptedComparables.length !== 1 ? "s" : ""} adopted
                    {adoptedComparables.length === 0 && " · Adopt comparables in the Adopted Comparables tab"}
                  </p>
                  {adoptedComparables.length === 0 ? (
                    <p style={{ fontSize: "12px", color: "#999", fontStyle: "italic" }}>No comparables adopted. Navigate to the Adopted Comparables tab to select evidence.</p>
                  ) : (
                    <table style={rptTable}>
                      <thead>
                        <tr>
                          {[
                            "Address", "Type / Era", "Hab. Rooms", "Price",
                            "Size (sq ft)", "£/sq ft", "Adj. £/sq ft",
                            ...(sizeElasticity > 0 ? [`Size-Adj PSF (β=${sizeElasticity}%)`] : []),
                            "Date",
                          ].map(h => (
                            <th key={h} style={rptTh}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {adoptedComparables.map((comp, i) => {
                          ...
                          const af        = adjFactors[i] ?? 1;
                          const adjPsf    = sqft != null && hpiTrend.length > 0 ? Math.round(comp.price * af / sqft) : null;
                          const sizeRes   = sizeAdjResults[i];
                          const sizeAdjFinalPsf = sizeRes?.adjPsf != null ? Math.round(sizeRes.adjPsf) : null;
                          return (
                            <tr key={comp.transaction_id ?? i} style={rptStripe(i)}>
                              <td style={{ ...rptTdV, fontStyle: "italic", fontWeight: 700, fontSize: "10px" }}>{comp.address}</td>
                              <td style={rptTdV}>{[comp.property_type, comp.building_era].filter(Boolean).join(", ") || "—"}</td>
                              <td style={{ ...rptTdV, textAlign: "center" }}>{comp.bedrooms ?? "—"}</td>
                              <td style={{ ...rptTdV, fontWeight: 700 }}>{formatPrice(comp.price)}</td>
                              <td style={{ ...rptTdV, textAlign: "right" }}>{sqft != null ? sqft.toLocaleString("en-GB") : "—"}</td>
                              <td style={{ ...rptTdV, textAlign: "right" }}>{psf != null ? `£${psf.toLocaleString("en-GB")}` : "—"}</td>
                              <td style={{ ...rptTdV, textAlign: "right", color: adjPsf != null ? "#0070C9" : "#999" }}>
                                {adjPsf != null ? `£${adjPsf.toLocaleString("en-GB")}` : "—"}
                              </td>
                              {sizeElasticity > 0 && (
                                <td style={{ ...rptTdV, textAlign: "right", color: sizeAdjFinalPsf != null ? "#5856D6" : "#999", fontWeight: sizeAdjFinalPsf != null ? 700 : undefined }}>
                                  {sizeAdjFinalPsf != null ? `£${sizeAdjFinalPsf.toLocaleString("en-GB")}` : "—"}
                                  {sizeRes?.capped && <span style={{ color: "#D97706", fontSize: "9px", marginLeft: "3px" }}>⚠</span>}
                                </td>
                              )}
                              <td style={rptTdV}>{fmtDateShort(comp.transaction_date)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                ...
                <div className="propval-report-section" style={rptSection}>
                  <h2 style={rptH2}>6. INDICATIVE VALUATION ANALYSIS</h2>

                  {adoptedComparables.length === 0 ? (
                    <p style={{ fontSize: "12px", color: "#999", fontStyle: "italic" }}>No comparable evidence adopted.</p>
                  ) : (
                    <>
                      {/* Comparable metrics summary table */}
                      <table style={{ ...rptTable, marginBottom: "16px" }}>
                        <thead>
                          <tr>
                            {["Metric", "Minimum", "Maximum", "Average"].map(h => <th key={h} style={rptTh}>{h}</th>)}
                          </tr>
                        </thead>
                        <tbody>
                          <tr style={rptStripe(0)}>
                            <td style={rptTdL}>Transaction Price</td>
                            <td style={rptTdV}>{fmtK(adoptedPriceMin)}</td>
                            <td style={rptTdV}>{fmtK(adoptedPriceMax)}</td>
                            <td style={{ ...rptTdV, fontWeight: 700 }}>{fmtK(adoptedPriceAvg)}</td>
                          </tr>
                          {adoptedPsfMin != null && (
                            <tr style={rptStripe(1)}>
                              <td style={rptTdL}>Price per sq ft</td>
                              <td style={rptTdV}>{fmtPsf(adoptedPsfMin)}</td>
                              <td style={rptTdV}>{fmtPsf(adoptedPsfMax!)}</td>
                              <td style={{ ...rptTdV, fontWeight: 700 }}>{fmtPsf(adoptedPsfAvg!)}</td>
                            </tr>
                          )}
                          {adjPsfMin != null && hpiCorrelation > 0 && hpiTrend.length > 0 && (
                            <tr style={rptStripe(2)}>
                              <td style={{ ...rptTdL, color: "#0070C9" }}>Adj. £/sq ft (HPI {hpiCorrelation}%)</td>
                              <td style={{ ...rptTdV, color: "#0070C9" }}>{fmtPsf(adjPsfMin)}</td>
                              <td style={{ ...rptTdV, color: "#0070C9" }}>{fmtPsf(adjPsfMax!)}</td>
                              <td style={{ ...rptTdV, fontWeight: 700, color: "#0070C9" }}>{fmtPsf(adjPsfAvg!)}</td>
                            </tr>
                          )}
                          {sizeAdjPsfMin != null && sizeElasticity > 0 && (
                            <tr style={rptStripe(3)}>
                              <td style={{ ...rptTdL, color: "#5856D6" }}>Size-Adj £/sq ft (β={sizeElasticity}%)</td>
                              <td style={{ ...rptTdV, color: "#5856D6" }}>{fmtPsf(sizeAdjPsfMin)}</td>
                              <td style={{ ...rptTdV, color: "#5856D6" }}>{fmtPsf(sizeAdjPsfMax!)}</td>
                              <td style={{ ...rptTdV, fontWeight: 700, color: "#5856D6" }}>{fmtPsf(sizeAdjPsfAvg!)}</td>
                            </tr>
                          )}
                          {adoptedSizeMin != null && (
                            <tr style={rptStripe(sizeElasticity > 0 ? 4 : 2)}>
                              <td style={rptTdL}>Floor Area (m²)</td>
                              <td style={rptTdV}>{Math.round(adoptedSizeMin)} m²</td>
                              <td style={rptTdV}>{Math.round(adoptedSizeMax!)} m²</td>
                              <td style={{ ...rptTdV, fontWeight: 700 }}>{Math.round(adoptedSizeAvg!)} m²</td>
                            </tr>
                          )}
                          {adoptedDateMin && (
                            <tr style={rptStripe(3)}>
                              <td style={rptTdL}>Transaction Date Range</td>
                              <td style={rptTdV}>{fmtDateShort(adoptedDateMin)}</td>
                              <td style={rptTdV}>{fmtDateShort(adoptedDateMax!)}</td>
                              <td style={rptTdV}>—</td>
                            </tr>
                          )}
                        </tbody>
                      ...
                      {/* Indicative value box */}
                      <div style={{ border: "1px solid #C6C6C8", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 12px rgba(0,122,255,0.12)" }}>
                        <div style={{ background: "linear-gradient(135deg, #007AFF 0%, #5856D6 100%)", padding: "10px 16px" }}>
                          <p style={{ color: "rgba(255,255,255,0.9)", fontSize: "11px", fontWeight: 600, margin: 0, letterSpacing: "0.04em", textTransform: "uppercase" }}>Indicative Market Value</p>
                        </div>
                        <div style={{ padding: "16px 20px" }}>
                          {indicativeValLow != null ? (
                            <>
                              {(() => {
                                const useSize = sizeElasticity > 0 && subjectAreaSqft != null && sizeAdjIndLow != null;
                                const useAdj  = !useSize && hpiCorrelation > 0 && hpiTrend.length > 0 && adjIndicativeLow != null;
                                ...
                                return (
                                  <>
                                    {subjectAreaM2 != null && (
                                      <p style={{ fontSize: "11px", color: "#6E6E73", margin: "0 0 10px" }}>
                                        Subject floor area: {Math.round(subjectAreaM2)} m² / {Math.round(subjectAreaM2 * 10.764).toLocaleString("en-GB")} sq ft
                                        {psf != null && <span> · {useSize ? "Size+time adj." : useAdj ? "Time-adj." : "Adopted"} rate: {fmtPsf(psf)} (avg of {adoptedWithArea.length} comp{adoptedWithArea.length !== 1 ? "s" : ""} with area{useSize ? `, HPI ${hpiCorrelation}%, β=${sizeElasticity}%` : useAdj ? `, HPI corr. ${hpiCorrelation}%` : ""})</span>}
                                      </p>
                                    )}
                                    <p style={{ fontSize: "26px", fontWeight: 700, color: "#007AFF", margin: "0 0 4px", letterSpacing: "-0.02em" }}>
                                      {fmtK(low)} – {fmtK(high)}
                                    </p>
                                    {mid != null && (
                                      <p style={{ fontSize: "13px", color: "#1D1D1F", margin: "0 0 12px" }}>
                                        Mid-point: <strong>{fmtK(mid)}</strong>
                                        {psf != null && <span style={{ color: "#86868B", fontSize: "11px" }}> · {fmtPsf(psf)} avg per sq ft</span>}
                                      </p>
                                    )}
                                  </>
                                );
                            ...
                            <p style={{ fontSize: "12px", color: "#86868B", fontStyle: "italic" }}>
                              {subjectAreaM2 == null
                          ...
                          <p style={{ fontSize: "10px", color: "#86868B", borderTop: "1px solid #E8E8ED", paddingTop: "10px", margin: "12px 0 0", lineHeight: 1.6 }}>
                            <strong style={{ color: "#6E6E73" }}>Disclaimer:</strong> This is a computer-generated indicative range based on adopted comparable evidence and is provided for guidance only. It does not constitute a formal opinion of value. A full inspection and valuation by a RICS Registered Valuer is required before any lending, purchase, or disposal decision is made. This output has not been prepared in accordance with RICS Valuation – Global Standards (Red Book) and must not be relied upon as such.
                          </p>
                        </div>
                      </div>
                    </>
                ...
                {/* Report footer */}
                <div style={{ borderTop: "1px solid #D2D2D7", paddingTop: "14px", marginTop: "8px" }}>
                  <p style={{ fontSize: "10px", color: "#86868B", margin: 0, lineHeight: 1.7 }}>
                    Generated by <strong>PropVal</strong> · {new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}
                    {" "}· Data sourced from HMLR Price Paid Data, EPC Open Data, Environment Agency, Historic England NHLE, Natural England, BGS GeoSure, and planning.data.gov.uk.
                    This report is for professional use only and does not constitute a formal RICS valuation.
                  </p>
                </div>
