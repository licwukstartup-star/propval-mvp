...
export default function Sub({ num, title, cats, children, onCatClick }: {
  ...
  return (
    <div className="rounded-md p-4" style={{ backgroundColor: "var(--color-bg-panel)", border: "1px solid var(--color-bg-surface)" }}>
      <div className="flex items-center gap-2 mb-3">
        ...
        {cats && <div className="flex gap-1 ml-auto">{cats.map(c => <CatBadge key={c} cat={c} onClick={onCatClick ? () => onCatClick(c) : undefined} />)}</div>}
      </div>
      {children}
    </div>
  )
